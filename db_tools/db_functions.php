...
function getRemarkReq(Mysqli $conn, $date="", $limit=""){
    $remarks = array();
    $query = <<<SQL
    SELECT StudentID, MarkerID, r.WorkID, Description, r.UpdateDate, Percentage
...
SQL;
    ...
    if($result){
        while ($row = $result->fetch_assoc()){
            $id = $row["MarkerID"];
            if(isset($remarks[$id])){
                array_push($remarks[$id], $row);
            } else {
                $remarks[$id] = array($row);
            }
        }
    }
    return $remarks;
...
function closeRemark(mysqli $conn, $user, $work){
    $query = <<<SQL
    UPDATE remark_request SET Actived=false
...
function deactive_request(mysqli $conn, $userid, $workid){
    $query = <<<SQL
    UPDATE remark_request
    SET Actived = false
    WHERE StudentID = '$userid' AND WorkID = '$workid';
SQL;
    return $conn -> query($query) !== false;
}

function submitRemark(mysqli $conn, $userAuth, $markerID, $workID, $reason){
    $query = <<<SQL
    INSERT INTO remark_request
    VALUES ('$userAuth', '$markerID','$workID', '$reason', True, NOW());
SQL;
